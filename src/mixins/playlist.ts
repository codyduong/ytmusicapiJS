...
export const PlaylistMixin = <TBase extends YTMusicBase>(Base: TBase) => {
  return class PlaylistMixin extends Base {
     ...
     * }
     */
    getPlaylist(playlistId: string, limit = 100): Array<any> {
      ...
      playlist['suggestions_token'] = nav(
        response, [...SINGLE_COLUMN_TAB, 'sectionListRenderer', 'contents', 1, ...MUSIC_SHELF, ...RELOAD_CONTINUATION, true]
      )

      playlist['tracks'] = []
      if (songCount > 0) {
        playlist['tracks'] = [...playlist['tracks'], ...parsePlaylistItems(results['contents'])]
        const songsToGet = Math.min(limit, songCount)

        if ('continuations' in results) {
          const requestFunc = (additionalParams) => await this._sendRequest(
